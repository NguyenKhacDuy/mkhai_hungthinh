    ...
    <el-table :data="arrLicense" style="width: 100%">
      ...
      <el-table-column align="right" width="130">
        <template slot-scope="scope">
          ...
          <el-button class="btn" style="margin: 5px 0px" @click="onApproveLicense(scope.row)" size="mini" type="primary">Phê duyệt</el-button>
          <el-button class="btn" size="mini" type="danger" @click="handleCancel(scope.row)">Không duyệt</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-drawer title="Lý do hủy yêu cầu duyệt chứng từ" :visible.sync="drawer">
...
<script>
import { getWaitingApproveLicense, approveLicense, cancelLicense } from "@/api/approver";
...
export default {
  ...
  methods: {
    ...
    onApproveLicense(license) {
      this.$confirm('Bạn có muốn phê duyệt chứng này không ?', 'Warning', {
    ...
    getWaitingLicense() {
      this.arrLicense = [];
      return getWaitingApproveLicense()
        .then(res => {
          if (res.status === 1) {
    ...
    approve(idLicense) {
      return approveLicense(idLicense).then(res => {
        if (res.status === 1) {
          this.$message({
            type: 'success',
